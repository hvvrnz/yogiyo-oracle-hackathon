...
def insert_package(rider_id, package_type, order_ids, route_detail, score,
                    score_detail, package_revenue, hourly_revenue):
    conn = get_connection()
    cursor = conn.cursor()
    package_id_var = cursor.var(int)
    cursor.execute("""
        INSERT INTO packages (
            rider_id, package_type, status, bundle_size, score,
            package_revenue, hourly_revenue,
            order_ids, route_detail, score_detail
        ) VALUES (
            :rider_id, :package_type, 'MATCHING', :bundle_size, :score,
            :package_revenue, :hourly_revenue,
            :order_ids, :route_detail, :score_detail
        )
