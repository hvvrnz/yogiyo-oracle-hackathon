...
지도는 현재 좌표 기반 SVG 렌더러입니다. 고객 지도는 매장·배달지·담당 라이더만, 라이더 지도는 전체 라이더 위치를 5초 폴링하며, 카카오맵 SDK 연결 전까지 fallback으로 사용합니다.
...
- 고객 주문 API에는 담당 `rider_id`, `package_id`가 없지만, 프론트가 매장명·좌표로 매장을 식별한 뒤 해당 매장 주문 목록에서 담당 라이더를 찾습니다.
