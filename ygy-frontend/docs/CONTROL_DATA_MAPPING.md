...
| 제어 또는 표시 | API | 응답 데이터 | 동작 |
| --- | --- | --- | --- |
| 매장 주문 목록 | `GET /api/merchant/{store_id}` | `orders[]`의 주문·조리시간·패키지·라이더 ID·경로, 선택적으로 라이더 이름·ETA | 주문 목록에 라이더 ID, 제공된 경우 라이더명·ETA, 한글 주문 상태와 배차/방문 순서를 렌더링 |
| 조리시간 수정 | `PUT /api/merchant/orders/{order_id}/cook-time` | `order_id`, `status=COOKING`, `owner_cook_min` | 성공 후 주문 목록 재조회 |
...
| 배차 제안 조회 | `GET /api/rider/{rider_id}/offers` | 아직 수락되지 않은 `OFFERED` 패키지 | 제안 카드로 표시. 같은 주문 조합의 여러 경로안은 구분 표기 |
| 배차 수락 | `PUT /api/rider/{rider_id}/package/{package_id}/accept` | `package_id`, `rider_id`, `MATCHING` | 성공 후 재조회. `409`는 다른 라이더가 먼저 수락한 정상 경쟁 결과로 안내 |
...
API 상태 변경은 패키지 단위다. 실제 모드에서는 배차 제안 조회·수락과 패키지 픽업·완료를 제공하며, 거절·만료 처리는 아직 제공하지 않는다.
...
| 최근 설명 조회 | `GET /api/explanation/{package_id}` | 고객·라이더 패키지 화면에 저장된 설명 표시 |
