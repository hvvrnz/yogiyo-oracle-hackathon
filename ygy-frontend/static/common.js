window.Yogiyo = (() => {
  ...
  const labels = {SINGLE:'단일 배달', AI_RECOMMENDED:'AI 추천 배달', SINGLE_DELIVERY:'개별 배달', AI_BUNDLE_3:'AI 추천 3건 묶음'};
  const stores = {
  ...
  const recalcEta = pkg => {
    ...
    pkg.route_steps.forEach(step => {
      ...
      if (step.type === 'PICKUP' && order.status !== 'READY') minutes += Math.max(0, order.remaining_cooking_min || 0);
  ...
  const makePackage = (orders, type) => {
    ...
    const route_steps = routeFor(orders); const isBundle = type === 'AI_BUNDLE_3';
    const pkg = {package_id:id, delivery_type:type, delivery_type_label:labels[type], order_ids:orders.map(order => order.order_id), status:'OFFERED', assigned_rider_id:null,
      ...
      bundle_reasons:isBundle ? ['세 매장의 조리 완료 예상 시각 차이가 허용 범위 안입니다.','매장과 고객 위치의 이동 동선이 겹칩니다.','세 주문 모두 음식 품질 제한 시간을 충족합니다.'] : [orders[0].delivery_preference === 'SINGLE' ? '고객이 단일 배달을 선택했습니다.' : '현재 AI 추천 주문이 2건 이하이므로 개별 배달로 진행합니다.'],
      route_steps, estimated_duration_min:route_steps.reduce((sum, step) => sum + step.duration_min, 0) + weather().travel_delay_min, total_distance_km:Number(route_steps.reduce((sum, step) => sum + step.distance_km,0).toFixed(1)), package_revenue:orders.reduce((sum, order) => sum + order.amount, 0), hourly_revenue:isBundle ? 28500 : 21000, route_strategy_label:isBundle ? 'AI 3건 묶음 최적화' : '개별 최적 경로', route_strategy_description:isBundle ? '조리 시간, 매장 위치, 고객 위치를 반영한 3건 묶음 배차입니다.' : '한 주문만 처리하는 개별 배달 경로입니다.'};
  ...
  const dispatch = () => {
    ...
    for (let index=0; index<ai.length; index+=3) { const group=ai.slice(index,index+3); created.push(makePackage(group, group.length === 3 ? 'AI_BUNDLE_3' : 'SINGLE_DELIVERY')); }
    // Groups of 1 or 2 must become individual packages, never a 2-order bundle.
    created.filter(pkg => pkg.delivery_type === 'SINGLE_DELIVERY' && pkg.order_ids.length > 1).forEach(pkg => { delete mock.packages[pkg.package_id]; pkg.order_ids.forEach(orderId => makePackage([mock.orders[orderId]], 'SINGLE_DELIVERY')); });
  ...
  const merchantView = storeId => {
    ...
    return {store:{...store,prediction_accuracy_pct:92,congestion:'보통'},summary:{new_count:orders.filter(o=>o.status==='NEW').length,cooking_count:orders.filter(o=>['COOKING','MATCHING','ACCEPTED'].includes(o.status)).length,ready_count:orders.filter(o=>o.status==='READY').length},orders,rider:{assigned:Boolean(rider),arrival_label:rider?`${rider.display_name} 매장 이동 중`:'라이더 배차 제안 중',remaining_min:rider?5:null,distance_km:rider?1.2:null,context:rider?`${rider.display_name}가 해당 주문을 배정받았습니다.`:'수락 후 AI 배차 계산을 실행하세요.'},package:pkg || {status_label:'배차 분석 대기',bundle_size:0,route_strategy_label:'-',ready_gap_min:0,total_wait_min:0,selected_route_reason:'수락한 주문의 고객 배송 선택을 분석합니다.'},weather:weather()};
  };
  ...
  const mockApi = (path, options={}) => {
    ...
    if (pathname === '/api/explanations/customer' || /^\/api\/explanations\//.test(pathname)) return {headline:'AI 추천 근거',summary:'조리 시간, 매장 위치, 고객 위치와 품질 제한을 함께 반영합니다.',note:'시연용 mock 데이터입니다.',source:'mock',reasons:[{title:'3건 조건',description:'AI 추천 주문은 정확히 3건일 때만 묶습니다.',metric:'정확히 3건'},{title:'이동 동선',description:'매장과 고객 위치를 반영합니다.',metric:'경로 최적화'},{title:'음식 품질',description:'예상 가방 체류시간을 확인합니다.',metric:'품질 기준'}]};
  ...
  const apiClient = Object.freeze({customer:{get:id=>api(`/api/customer/${id}`)},orders:{create:body=>api('/api/orders',{method:'POST',body:JSON.stringify(body)})},merchant:{get:id=>api(`/api/merchant/${id}`),orderAction:(id,body)=>api(`/api/merchant/orders/${id}/action`,{method:'POST',body:JSON.stringify(body)})},rider:{get:id=>api(`/api/rider/${id}`),offerResponse:(riderId,packageId,body)=>api(`/api/rider/${riderId}/packages/${packageId}/offer-response`,{method:'POST',body:JSON.stringify(body)}),pickup:(riderId,orderId)=>api(`/api/rider/${riderId}/orders/${orderId}/pickup`,{method:'POST',body:'{}'}),deliver:(riderId,orderId)=>api(`/api/rider/${riderId}/orders/${orderId}/deliver`,{method:'POST',body:'{}'})},explanation:(role,id)=>api(`/api/explanations/${role}/${id}`),maps:{config:()=>api('/api/config/maps')},demo:{state:()=>api('/api/state'),dispatchCalculate:()=>api('/api/demo/dispatch-calculate',{method:'POST',body:'{}'}),reset:()=>api('/api/demo/reset',{method:'POST',body:'{}'}),weather:body=>api('/api/demo/weather',{method:'POST',body:JSON.stringify(body)})}});
  ...
  const websocket=(role,entityId,onUpdate)=>{if(useMock){setConnection(true);const listener=e=>{if(e.key===storageKey&&e.newValue){hydrate();onUpdate({type:'mock_state_updated',role,entityId});}};window.addEventListener('storage',listener);return()=>window.removeEventListener('storage',listener);} const socket=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/ws/${role}/${entityId}`);socket.onopen=()=>setConnection(true);socket.onmessage=()=>onUpdate({});return()=>socket.close();};
  ...
  return {qs,el,money,fmtTime,escape,api,apiUrl:path=>`${apiBaseUrl}${path}`,apiClient,defaultIds:ids,toast,websocket,openSheet,closeSheet,renderRouteMap,useMock,labels};
})();
