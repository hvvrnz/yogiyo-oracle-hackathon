...
async function loadDatasets() {
  const payload = await Yogiyo.api('/api/demo/datasets');
...
async function loadDemo() {
  try {
    ...
    const state = await Yogiyo.api('/api/state');
    ...
    const activeRiderId = pkg.rider_id || pkg.offered_rider_id || 'R-001';
...
async function post(endpoint, body) {
  try {
    const result = await Yogiyo.api(endpoint, {method:'POST', body:JSON.stringify(body || {})});
    await loadDemo();
    return result;
...
Yogiyo.el('applyDataset').addEventListener('click', async () => {
  ...
  if (!datasetId) return;
  await post('/api/demo/dataset', {dataset_id: datasetId});
  datasetsLoaded = false;
  await loadDemo();
});
document.querySelectorAll('[data-endpoint]').forEach(button => button.addEventListener('click', () => post(button.dataset.endpoint)));
document.querySelectorAll('[data-route-strategy]').forEach(button => button.addEventListener('click', () => post('/api/demo/route-strategy',{strategy:button.dataset.routeStrategy})));
document.querySelectorAll('[data-weather]').forEach(button => button.addEventListener('click', () => post('/api/demo/weather',{condition:button.dataset.weather})));
document.querySelectorAll('[data-simulation]').forEach(button => button.addEventListener('click', () => post('/api/demo/simulation',{running:button.dataset.simulation === 'true'})));
