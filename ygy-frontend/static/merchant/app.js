let storeId = Yogiyo.qs('storeId', Yogiyo.defaultIds.merchant);
let currentView = null;
let explanation = null;

async function loadMerchant(silent=false) {
  try {
    currentView = await Yogiyo.apiClient.merchant.get(storeId);
...
async function handleOrderAction(event) {
  ...
  try {
    const result = await Yogiyo.apiClient.merchant.orderAction(button.dataset.order, body);
...
async function showExplanation() {
  ...
  try { explanation = await Yogiyo.apiClient.explanation('merchant', storeId); }
