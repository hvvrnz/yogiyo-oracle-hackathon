const riderId = Yogiyo.qs('riderId', 'R-001');
let currentView = null;

async function loadRider() {
  try { currentView = await Yogiyo.api(`/api/rider/${riderId}`); renderRider(currentView); }
...
async function handleRiderAction(event) {
  ...
  try {
    const result = await Yogiyo.api(`/api/rider/${riderId}/action`, {method:'POST', body:JSON.stringify({action:button.dataset.riderAction})});
...
async function showExplanation() {
  ...
  try {
    const explanation = await Yogiyo.api(`/api/explanations/rider/${riderId}`);
