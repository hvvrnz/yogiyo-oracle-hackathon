const customerId = Yogiyo.qs('customerId', 'C-001');
let currentView = null;
let explanation = null;

async function loadCustomer(silent=false) {
  try {
    currentView = await Yogiyo.api(`/api/customer/${customerId}`);
...
async function showExplanation() {
  ...
  if (!explanation) {
    try { explanation = await Yogiyo.api(`/api/explanations/customer/${customerId}`); }
